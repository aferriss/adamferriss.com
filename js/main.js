function Project(url, name, id, visible, tags, video, poster){
	this.id = id;
	this.url = url;
	this.name = name;
...
var jxhr = $.getJSON('assets/projects.json', function(data){
	var items = [];
	var projects = [];
	var videos = [];

	$.each(data, function(key, val){
		items.push('<li class="project" id="'+key+'"><a href="' + val.url + '">' + val.name + '</a></li>');
		var project = new Project(val.url, val.name, key, false, val.tags, 'assets/' + val.video, 'assets/' + val.poster);

		projects.push(project);
		// if(val.video != undefined){
		// 	videos.push('../assets/' + val.video);
		// }
	...
	$('.projectList').append('<li id="infoLink">Info</li>');
	...
	var vidC = document.getElementsByClassName('videoDiv');
	...
	if(!mobile){
		...
		for(var i = 0; i<vidC.length; i++){
			vidC[i].style.margin = '10px';
			vidC[i].childNodes[0].style.visibility = 'visible';
		}
		var home = document.getElementById('home')
		home.style.mixBlendMode = 'normal';
		home.style.color = 'black';

		var info = document.getElementById('link')
